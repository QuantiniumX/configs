...
echo "Doing a system update and installing base-devel wget git and jetbrains mono font"
sudo pacman -Syyu --noconfirm --needed base-devel wget git ttf-jetbrains-mono-nerd
...
sudo pacman -S --noconfirm --needed rofi feh xorg xorg-xinit xorg-xinput $DRI xmonad xwallpaper
...
echo "Do you have an old picom directory in ~/.src/   1) yes   2) no"
read -r -p "[YES(1) / NO(2)]: "  ans

if [ $ans -eq 1 ]
then
    rm -rf ~/.src/picom
    echo "Deleted ~/.src/picom as it would conflict"
fi
...
mkdir -p ~/.config/
  ...
  ;;
    ...
    if [ -d ~/wallpapers ]; then
        ...
        cp ./wallpapers/garden_girl.png ~/wallpapers/;
    else
